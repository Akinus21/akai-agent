...
impl TunnelClient {
    ...
    fn build_tls_connector(&self) -> Result<TlsConnector> {
        ...
        let client_certs = rustls_pemfile::certs(&mut &self.client_cert_pem[..])
            .collect::<Result<Vec<_>, _>>()
            .context("failed to parse client cert")?;

        let client_key = rustls_pemfile::private_key(&mut &self.client_key_pem[..])
...
async fn serve_conn(
    ...
    let down = async move {
        while let Some(data) = write_rx.recv().await {
            ...
            if rpc_writer.write_all(&data).await.is_err() {
                break;
            }
